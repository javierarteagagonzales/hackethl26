...
export function CallsSection({ t, isDark }: { t: (key: string) => string, isDark: boolean }) {
  ...
  return (
    <section id="calls" className="py-16 md:py-24 border-t border-border relative z-10 bg-surface/30">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16 flex flex-col items-center">
          <h2 className="text-3xl md:text-5xl font-bold tracking-tight" style={{ letterSpacing: "-0.02em" }}>
            {t("calls.title")} <span className="text-gradient-sunset font-extrabold">{t("calls.accent")}</span>
          </h2>
          <p className="text-fg/70 text-lg max-w-2xl mx-auto mt-4 font-light">{t("calls.description")}</p>
        </div>
          ...
          {callsData.map((call, index) => {
            ...
            const cardContent = (
              <>
                ...
                <div className={`w-14 h-14 rounded-2xl bg-surface/80 border border-border flex items-center justify-center mb-6 shadow-sm ${isActive ? 'group-hover:border-brand-accent/30 group-hover:scale-110 transition-all' : ''}`}>
                ...
                <div className={`mt-auto inline-flex items-center gap-2 font-mono text-sm uppercase tracking-wider font-semibold ${isActive ? 'text-brand-accent group-hover:gap-3 transition-all' : 'text-fg/40 cursor-not-allowed'}`}>
