...
export function FAQSection({ t, tArray }: { t: (key: string) => string, tArray: (key: string) => any[] }) {
    ...
    <section id="faq" className="py-16 md:py-24 border-t border-border relative z-10 bg-surface/30">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16 flex flex-col items-center">
          <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-4" style={{ letterSpacing: "-0.02em" }}>
            {t("faq.title")} <span className="text-gradient-sunset font-extrabold">{t("faq.title_accent")}</span>
          </h2>
        ...
        <div className="max-w-3xl mx-auto">
          <div className="space-y-4">
            {previewQuestions.map((item: any, i: number) => (
              <motion.details
                key={i}
