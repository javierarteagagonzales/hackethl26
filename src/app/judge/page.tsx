...
import { JudgeSkeleton } from "@/components/dashboard-skeletons";
import Link from "next/link";
import { signOut } from "next-auth/react";
import type { Prisma } from "@prisma/client";

type ProjectForJudging = Prisma.ProjectGetPayload<{
  include: {
    team: { include: { members: { select: { name: true } } } };
    track: true;
    evaluations: true;
  };
}>;
...
const criteria: { id: keyof EvaluationInput; label: string; desc: string }[] = [
...
export default function JudgePage() {
  ...
  const [projects, setProjects] = useState<ProjectForJudging[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedProject, setSelectedProject] = useState<ProjectForJudging | null>(null);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- session data object is stable after auth
  }, [status, router]);

  const openEvaluation = (project: ProjectForJudging) => {
    ...
    return (
      <div className="flex h-screen bg-black text-white font-sans overflow-hidden">
        ...
        <main className="flex-1 flex flex-col min-w-0 overflow-y-auto">
          <header className="h-16 border-b border-white/5 flex items-center px-8 bg-black/40 backdrop-blur-md sticky top-0 z-10">
            <div className="h-4 w-48 bg-white/5 animate-pulse rounded" />
          </header>
          <div className="p-8 space-y-8">
            <div className="space-y-2">
              <div className="h-9 w-64 bg-white/5 animate-pulse rounded" />
              <div className="h-4 w-96 bg-white/5 animate-pulse rounded" />
            </div>
            <JudgeSkeleton />
          </div>
        </main>
    ...
    <div className="flex h-screen bg-black text-white font-sans overflow-hidden">
      ...
      <aside className="w-64 border-r border-white/5 flex flex-col shrink-0 bg-[#080808]">
        ...
        <nav className="flex-1 p-4 space-y-2 mt-4">
          <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 transition-all duration-300">
        ...
        <div className="p-6 border-t border-white/5 space-y-4">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-yellow-600 to-orange-600 flex items-center justify-center text-xs font-bold ring-2 ring-yellow-500/20">
              {session?.user?.name?.[0] || "J"}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold truncate">{session?.user?.name || "Judge"}</p>
              <p className="text-[10px] text-gray-500 uppercase tracking-widest">Main Evaluator</p>
            </div>
          </div>
          <button
            onClick={() => signOut({ callbackUrl: "/" })}
            className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs text-gray-500 hover:text-red-400 hover:bg-red-400/5 transition-all"
          >
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        <header className="h-16 border-b border-white/5 flex items-center justify-between px-8 bg-black/40 backdrop-blur-md sticky top-0 z-10">
          <h2 className="text-sm font-bold tracking-tight uppercase font-mono text-gray-400">
            {"// Judging Terminal "}
            <span className="text-white mx-2">/</span> Projects List
          </h2>
        ...
        <div className="p-8">
          <div className="mb-10">
            ...
            <p className="text-gray-500 text-sm mt-1">
              Review and score projects based on innovation, design, and impact.
            </p>
          </div>
              ...
              >
                <div className="p-6 border-b border-white/5 bg-white/2 flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-blue-400 font-mono uppercase mt-1">
                      Track: {proj.track?.title || "General"}
                    </p>
                  </div>
                ...
                <div className="p-6 flex-1 flex flex-col space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    {proj.githubUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="p-1.5 rounded bg-white/5 border border-white/10 text-gray-500 hover:text-white transition-colors"
                      >
                  ...
                  <div className="pt-4 border-t border-white/5 mt-auto">
                    ...
                    >
                      {proj.evaluations?.length > 0 ? "Edit Evaluation" : "Start Evaluation"}{" "}
              ...
              <div className="p-6 border-b border-white/5 flex items-center justify-between sticky top-0 bg-[#0c0c0c] z-10">
                <div>
                  ...
                  <p className="text-xs text-gray-500 font-mono">
                    Evaluate technical and impact criteria
                  </p>
                </div>
                <button
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-8">
                ...
                <div className="space-y-3 pt-4 border-t border-white/5">
                  <label className="text-sm font-bold text-white flex items-center gap-2">
                    <MessageSquare className="w-3 h-3 text-gray-500" /> Judge Comments
                  </label>
                  <textarea
