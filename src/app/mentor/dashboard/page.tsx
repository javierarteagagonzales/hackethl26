...
type MentorshipWithTeam = Prisma.MentorshipGetPayload<{
  include: {
    team: {
      include: {
...
export default function MentorDashboard() {
  ...
  const [mentorships, setMentorships] = useState<MentorshipWithTeam[]>([]);
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-white/5">
        <div>
          <h1 className="text-3xl font-black tracking-tighter">
            Mentorship <span className="text-blue-500">Requests</span>
          </h1>
          <p className="text-gray-500 text-sm mt-1">Manage and assist teams in real-time.</p>
        </div>
        <div className="flex gap-2">
          <Badge variant="outline" className="bg-white/5 border-white/10 px-3 py-1 text-[10px]">
            {mentorships.filter((m) => m.status === "PENDING").length} Pending
          </Badge>
          <Badge variant="outline" className="bg-white/5 border-white/10 px-3 py-1 text-[10px]">
            {mentorships.filter((m) => m.status === "IN_PROGRESS").length} In Progress
          </Badge>
        </div>
      </header>
      ...
      {mentorships.length === 0 ? (
        <div className="h-[400px] flex flex-col items-center justify-center border border-dashed border-white/5 rounded-2xl bg-white/1">
          ...
          <p className="text-gray-600 font-mono text-xs uppercase tracking-widest">
            Quiet terminal... no requests yet.
          </p>
        </div>
      ) : (
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge
                        variant="secondary"
                        className="bg-white/5 text-[10px] uppercase font-mono"
                      >
                        {m.status}
                      </Badge>
                    </div>
                    <div className="flex items-center gap-4 text-xs text-gray-500">
                      <span className="flex items-center gap-1.5">
                        <Users className="w-3 h-3" /> Team: {m.team?.name}
                      </span>
                      <span className="flex items-center gap-1.5">
                        <Calendar className="w-3 h-3" /> {new Date(m.scheduledAt).toLocaleString()}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="px-6 py-4 bg-white/2 border-t border-white/5 flex flex-wrap gap-4 items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  {m.team?.members.map((member) => (
                    <div
                      key={member.id}
                      className="flex items-center gap-1.5 bg-black/40 border border-white/5 px-2 py-1 rounded text-[10px] text-gray-400"
                    >
                ...
                {m.team?.project && (
                  <Link
                    href={`/projects/${m.team.project.id}`}
