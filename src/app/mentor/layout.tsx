...
import { Calendar, MessageSquare, Settings, LogOut, Video } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col md:flex-row overflow-hidden h-screen">
      <aside className="w-full md:w-64 border-r border-white/5 flex flex-col sticky top-0 md:h-screen shrink-0 bg-[#080808]">
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          <Link
            ...
            className="flex items-center gap-3 px-3 py-2 rounded-xl bg-white/5 text-white font-medium border border-white/10"
          >
            <Calendar className="w-4 h-4" /> Requests
          </Link>
          <Link
            href="/mentor/settings"
            className="flex items-center gap-3 px-3 py-2 rounded-xl text-gray-500 hover:text-white hover:bg-white/5 transition-colors"
          >
        ...
        <div className="p-6 border-t border-white/5 space-y-4">
          ...
          <Button
            ...
            className="w-full justify-start text-xs text-gray-500 hover:text-red-400 hover:bg-red-400/5"
          >
      ...
      <main className="flex-1 overflow-y-auto bg-[#020202]">
        <div className="container max-w-6xl mx-auto p-6 md:p-10">{children}</div>
      </main>
    </div>
  );
}
