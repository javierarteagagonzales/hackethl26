...
import { ArrowLeft, Shield, Database, Lock, Eye, Users, Mail, Globe } from "lucide-react";
...
const sectionIcons: Record<string, React.ReactNode> = {
  data_collection: <Database className="w-5 h-5" />,
  data_use: <Eye className="w-5 h-5" />,
  data_sharing: <Users className="w-5 h-5" />,
  data_security: <Lock className="w-5 h-5" />,
  cookies: <Globe className="w-5 h-5" />,
  user_rights: <Shield className="w-5 h-5" />,
  ...
  return (
    <div className="min-h-screen bg-bg text-fg font-sans transition-colors duration-300">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none opacity-20 hidden lg:block"
        style={{
          background:
            "radial-gradient(ellipse 70% 50% at 10% 0%, rgba(230,74,48,0.18), transparent 60%), radial-gradient(ellipse 60% 50% at 90% 10%, rgba(61,190,213,0.14), transparent 60%)",
        }}
      />
        ...
        <div className="container mx-auto px-6 py-16 md:py-24">
          ...
          >
            ...
            <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-4 leading-tight">
              {t("privacy.title")}{" "}
              <span className="text-gradient-sunset">{t("privacy.title_accent")}</span>
            </h1>
            <p className="text-fg/60 text-lg leading-relaxed max-w-2xl">
              {t("privacy.subtitle")}
            </p>
            <p className="text-fg/40 text-sm font-mono mt-4">
              {t("privacy.last_updated")}: <span className="text-brand-accent">2026-06-24</span>
            </p>
          </motion.div>
        </div>
      </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-8">
            {sectionKeys.map((key, i) => {
              const items = tArray(`privacy.sections.${key}.items`);
              return (
                <motion.div
                  key={key}
                  initial={{ opacity: 0, y: 20 }}
          ...
          >
            <div>
              <p className="font-semibold text-fg mb-1">{t("privacy.cta_title")}</p>
              <p className="text-fg/50 text-sm">{t("privacy.cta_desc")}</p>
            </div>
            <a
      ...
      <footer className="border-t border-border bg-surface/40 py-8 mt-8">
        <div className="container mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-fg/40">
          ...
          <p className="font-mono text-xs">
            © 2026 Ethereum Lima. {t("privacy.all_rights")}
          </p>
