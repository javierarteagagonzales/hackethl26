...
type TeamWithMembers = Prisma.TeamGetPayload<{
  include: { members: { select: { id: true; name: true; email: true; github: true } } };
}>;

type MentorshipWithMentor = Prisma.MentorshipGetPayload<{
  include: { mentor: { select: { name: true; github: true } } };
}>;

type TrackWithRelations = Prisma.TrackGetPayload<{
  include: { sponsor: true; categories: true; prizes: true };
}>;
...
export default function DashboardPage() {
  ...
  const [mentorships, setMentorships] = useState<MentorshipWithMentor[]>([]);
  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      ...
      if (session?.user?.role === "ADMIN" || session?.user?.role === "SUPERADMIN") {
        ...
        // eslint-disable-next-line react-hooks/set-state-in-effect -- client-only data fetch on mount
        fetchInitialData();
      }
    }
  ...
  if (status === "loading") {
    return (
      <div className="h-screen bg-black flex items-center justify-center text-white font-mono tracking-widest uppercase text-xs">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-white/5">
        <div>
          <h1 className="text-3xl font-black tracking-tighter">
            Welcome, <span className="text-brand-blue">{session.user?.name || "Hacker"}</span> 👾
          </h1>
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <section className="bg-white/2 border border-white/8 rounded-xl overflow-hidden">
            <div className="p-6 border-b border-white/5 flex items-center justify-between">
              ...
              {team && (
                <button onClick={handleLeaveTeam} className="text-xs text-red-500 hover:underline">
                  Leave Team
                </button>
              )}
            </div>
            ...
            {loading ? (
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      {team.description || "Building something amazing at ETH Lima."}
                    </p>
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {team.members.map((member) => (
                      <div
                        key={member.id}
                        className="p-3 rounded-lg border border-white/5 bg-white/2 flex items-center justify-between group"
                      >
                        <div className="flex items-center gap-3">
              ...
              <div className="p-12 text-center">
                ...
                <p className="text-gray-500 text-sm mb-8 max-w-sm mx-auto">
                  Hackathons are better with a squad. Create a new team to start submitting a
                  project.
                </p>
                ...
                <div className="flex flex-col gap-6 w-full max-w-md mx-auto">
                  <form onSubmit={teamForm.handleSubmit(handleCreateTeam)} className="flex gap-2">
                    <div className="flex-1">
                      <input
                        ...
                        className="w-full h-10 px-3 rounded-lg bg-white/5 border border-white/10 text-sm focus:outline-none focus:border-blue-500/30"
                      />
                    ...
                    <button
                      type="submit"
                      className="h-10 px-6 rounded-lg bg-white text-black font-bold text-sm hover:bg-gray-200 transition-colors whitespace-nowrap"
                    >
                      Create Team
                    </button>
                  </form>
                  ...
                  <div className="relative py-2">
                    ...
                    <div className="relative flex justify-center text-[10px] uppercase font-mono text-gray-700 bg-transparent">
                      <span className="px-2 bg-black">OR JOIN WITH CODE</span>
                    </div>
                  </div>
                  ...
                  <form onSubmit={joinForm.handleSubmit(handleJoinTeam)} className="flex gap-2">
                    <div className="flex-1">
                      <input
                        ...
                        className="w-full h-10 px-3 rounded-lg bg-white/5 border border-white/10 text-sm focus:outline-none focus:border-blue-500/30 uppercase"
                      />
                    ...
                    <button
                      type="submit"
                      disabled={joining}
                      className="h-10 px-6 rounded-lg border border-white/10 text-white font-bold text-sm hover:bg-white/5 transition-colors disabled:opacity-50"
                    >
          ...
          <section className="bg-white/2 border border-white/8 rounded-xl overflow-hidden">
            <div className="p-6 border-b border-white/5 flex items-center justify-between">
              ...
              {project && (
                <Badge className="bg-green-500/10 text-green-500 border-green-500/20 uppercase text-[10px]">
                  Submitted
                </Badge>
              )}
            </div>
            ...
            {loading ? (
              ...
              <div className="p-12 text-center">
                <p className="text-gray-600 text-sm">
                  Join or create a team to start building your project.
                </p>
              </div>
            ) : project ? (
              <div className="p-6 space-y-6">
              ...
              <form onSubmit={projectForm.handleSubmit(handleSubmitProject)} className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-1.5">
                    <label className="text-xs text-gray-500 font-mono uppercase ml-1">
                      Project Name
                    </label>
                    <input
                      {...projectForm.register("name")}
                      placeholder="My Awesome DApp"
                      className="w-full h-11 px-4 rounded-lg bg-black border border-white/10 text-sm focus:outline-none focus:border-blue-500/50 transition-all"
                    />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select
                      {...projectForm.register("trackId")}
                      className="w-full h-11 px-4 rounded-lg bg-black border border-white/10 text-sm text-gray-400 focus:outline-none focus:border-blue-500/50 transition-all appearance-none"
                    >
                    ...
                    {projectForm.formState.errors.trackId && (
                      <p className="text-[10px] text-red-500 mt-1">
                        {projectForm.formState.errors.trackId.message}
                      </p>
                    )}
                  </div>
                </div>
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    rows={4}
                    placeholder="What does your project do? How was it built?"
                    className="w-full px-4 py-3 rounded-lg bg-black border border-white/10 text-sm focus:outline-none focus:border-blue-500/50 transition-all resize-none"
                  />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full h-11 px-4 rounded-lg bg-black border border-white/10 text-sm focus:outline-none focus:border-blue-500/50 transition-all"
                    />
                    {projectForm.formState.errors.githubUrl && (
                      <p className="text-[10px] text-red-500 mt-1">
                        {projectForm.formState.errors.githubUrl.message}
                      </p>
                    )}
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full h-11 px-4 rounded-lg bg-black border border-white/10 text-sm focus:outline-none focus:border-blue-500/50 transition-all"
                    />
                    {projectForm.formState.errors.demoUrl && (
                      <p className="text-[10px] text-red-500 mt-1">
                        {projectForm.formState.errors.demoUrl.message}
                      </p>
                    )}
                  </div>
                </div>
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full h-11 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                >
            ...
            <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="space-y-4">
                <h3 className="text-sm font-bold flex items-center gap-2">
                  <HelpCircle className="w-4 h-4 text-purple-400" /> Request a Mentor
                </h3>
                <p className="text-xs text-gray-500 leading-relaxed">
                  Stuck on a bug or need architecture advice? Our mentors are here to help you cross
                  the finish line.
                </p>

                <form
                  onSubmit={mentorshipForm.handleSubmit(handleRequestMentorship)}
                ...
                >
                  <div className="space-y-1.5">
                    ...
                    <input
                      {...mentorshipForm.register("topic")}
                      placeholder="Smart contract debug, UX feedback..."
                      className="w-full h-9 px-3 rounded-lg bg-black border border-white/10 text-xs focus:outline-none focus:border-purple-500/30"
                    />
                    {mentorshipForm.formState.errors.topic && (
                      <p className="text-[9px] text-red-500 mt-0.5">
                        {mentorshipForm.formState.errors.topic.message}
                      </p>
                    )}
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <input
                      {...mentorshipForm.register("scheduledAt")}
                      type="datetime-local"
                      className="w-full h-9 px-3 rounded-lg bg-black border border-white/10 text-xs text-gray-400 focus:outline-none focus:border-purple-500/30"
                    />
                    {mentorshipForm.formState.errors.scheduledAt && (
                      <p className="text-[9px] text-red-500 mt-0.5">
                        {mentorshipForm.formState.errors.scheduledAt.message}
                      </p>
                    )}
                  </div>
                  <button
                    type="submit"
                    disabled={requestingMentorship || !team}
                    className="w-full h-9 rounded-lg bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs transition-all disabled:opacity-50"
                  >
                  ...
                  {mentorships.length === 0 ? (
                    ...
                    mentorships.map((m) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`p-2 rounded-lg ${m.status === "PENDING" ? "bg-yellow-500/10 text-yellow-500" : "bg-green-500/10 text-green-500"}`}
                          >
                            {m.status === "PENDING" ? (
                          ...
                          <div>
                            ...
                            <div className="text-[10px] text-gray-500">
                              Mentor: {m.mentor?.name || "Assigning..."}
                            </div>
                          </div>
                        </div>
                        <Badge
                          className={`text-[8px] uppercase tracking-tighter ${m.status === "PENDING" ? "bg-yellow-500/10 text-yellow-500" : "bg-green-500/10 text-green-500"}`}
                        >
                          {m.status}
                        </Badge>
                      </div>
        ...
        <div className="space-y-6">
          ...
          <section className="bg-white/2 border border-white/8 rounded-xl p-6 space-y-4">
            ...
            <div className="space-y-2">
              {[
                { label: "Official Rulebook", href: "/conduct" },
                { label: "Arbitrum Docs", href: "https://docs.arbitrum.io/" },
                { label: "Technical Support", href: "https://t.me/javierdgtl" },
              ].map((link) => (
                <a
                  key={link.label}
                  href={link.href}
                  ...
                  className="flex items-center justify-between text-sm text-gray-400 hover:text-white transition-all py-2 group"
                >
                  {link.label}{" "}
