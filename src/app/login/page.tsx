...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-bg text-fg flex flex-col md:flex-row font-sans overflow-hidden transition-colors duration-300 relative">
      ...
      <div
        className="fixed inset-0 z-0 opacity-10 pointer-events-none md:hidden"
        style={{
          backgroundImage:
            "radial-gradient(circle at 1px 1px, var(--color-brand-accent) 1px, transparent 0)",
      ...
      <div className="hidden md:flex md:w-1/2 bg-surface/50 border-r border-border relative flex-col justify-between p-12 overflow-hidden">
        ...
        <div className="relative z-10 text-xs text-fg/40 font-mono">{t("login.all_rights")}</div>
      </div>
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="space-y-4">
            <button
              ...
              className="w-full h-11 rounded-lg bg-surface border border-border flex items-center justify-center gap-3 hover:bg-fg/5 transition-all group cursor-pointer"
            >
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-bg px-4 text-fg/40 font-mono">{t("login.or_email")}</span>
            </div>
          </div>

          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs text-fg/60 font-medium ml-1">
                {t("login.email_label")}
              </label>
              <input
                name="email"
                ...
                required
                className="w-full h-11 px-4 rounded-lg bg-surface border border-border text-sm focus:outline-none focus:border-brand-accent/50 transition-all placeholder:text-fg/30"
              />
            </div>
            <div className="space-y-1.5">
              <div className="flex items-center justify-between ml-1">
                <label className="text-xs text-fg/60 font-medium">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full h-11 rounded-lg bg-fg hover:opacity-90 text-bg font-bold text-sm transition-all flex items-center justify-center gap-2 group disabled:opacity-50 cursor-pointer shadow-md"
            >
          ...
          <div className="p-4 rounded-lg bg-brand-accent/5 border border-brand-accent/10 text-[10px] text-fg/55 space-y-1 leading-relaxed font-mono">
            <p className="font-bold text-brand-accent uppercase tracking-widest mb-1">
              {"// "}
              {t("login.sandbox_mode")} {"(Password: password123)"}
            </p>
